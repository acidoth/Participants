...
# under the License.
#
"""Implementation of non-blocking coordinationserver.

The main idea of the coordinationserver is to receive and send requests
...
from thriftlib.protocol.TBinaryProtocol import TBinaryProtocolFactory
...
class TNonblockingServer:
    """Non-blocking coordinationserver."""
    ...
    def prepare(self):
        """Prepares coordinationserver for serve requests."""
        if self.prepared:
            return
        self.socket.listen()
    ...
    def wake_up(self):
        ...
        The coordinationserver usualy waits in select call in we should terminate one.
    ...
    def stop(self):
        """Stop the coordinationserver.

        This method causes the serve() method to return.  stop() may be invoked
        from within your handler, or from another thread.

        After stop() is called, serve() will return but the coordinationserver will still
        ...
        serve() returns to close the coordinationserver socket and shutdown all worker
        threads.
        """
        self._stop = True
        self.wake_up()
