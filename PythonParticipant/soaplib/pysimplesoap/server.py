...
import logging
from soaplib.pysimplesoap.simplexml import SimpleXMLElement, TYPE_MAP, Date, Decimal
...
if __name__=="__main__":
    ...
    if '--serve' in sys.argv:
        print "Starting coordinationserver..."
        httpd = HTTPServer(("", 8008), SOAPHandler)
        httpd.dispatcher = dispatcher
        httpd.serve_forever()

    if '--consume' in sys.argv:
        from soaplib.pysimplesoap.client import SoapClient
        client = SoapClient(
            location = "http://localhost:8008/",
            action = 'http://localhost:8008/', # SOAPAction
