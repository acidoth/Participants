...
public class Repository {
     ...
     * must be logged or not. This constant is intended to be used only by the ARepository and the
     * Embedded Registry.
     */
    public static final String IS_LOGGING_ACTIVITY = "isLoggingActivity";
...
//        We need to change the UUID of the new resource
    ...
    public void restore(String _path, Reader reader)
        ...
        try {
            restoreRecursively(path, xmlReader, dumpReader, currentVersion);
    ...
    private void restoreRecursively(String path,
        ...
        synchronized (this){
            ...
            if(resourceDAO.resourceExists(path)){
                ...
                long resourceVersion = resourceDO.getVersion();
                if(resourceVersion != dumpingResourceVersion && !DumpConstants.RESOURCE_DELETED.equals(status)){
                    status = DumpConstants.RESOURCE_UPDATED;
                }
            } else {
                status = DumpConstants.RESOURCE_ADDED;
            }
            if(status != null){
                if(resourceDAO.resourceExists(path)){
                    deleteNode(resourceID, resourceDO, true);
