...
public class JDBCTransactionManager implements TransactionManager {
    ...
    public void rollbackTransaction(Connection connection) throws RegistryException {
        dataAccessManager.getDatabaseTransaction().setRollbacked(true);
        if (dataAccessManager.getDatabaseTransaction().getNestedDepth() != 1) {
            if (log.isTraceEnabled()) {
                log.trace(
        ...
        //JDBCDatabaseTransaction.ManagedRegistryConnection conn =
                //JDBCDatabaseTransaction.getConnection();
        JDBCDatabaseTransaction.ManagedRegistryConnection conn= (JDBCDatabaseTransaction.ManagedRegistryConnection) connection;
        try {
            conn.rollback();
    ...
    public void commitTransaction(Connection connection) throws RegistryException {
        if (dataAccessManager.getDatabaseTransaction().getNestedDepth() != 1) {
            if (log.isTraceEnabled()) {
                log.trace(
                        "The transaction was not committed, because it is called within a " +
    ...
    private void endTransaction(Connection connection) throws RegistryException {

        if (Transaction.isStarted()) {
            //JDBCDatabaseTransaction.ManagedRegistryConnection conn =   connection;
            //        JDBCDatabaseTransaction.getConnection();
