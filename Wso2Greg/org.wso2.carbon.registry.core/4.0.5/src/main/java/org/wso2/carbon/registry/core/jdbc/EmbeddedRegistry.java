...
public class EmbeddedRegistry implements Registry {
    ...
    protected static final Log log = LogFactory.getLog(EmbeddedRegistry.class);
    ...
    protected DataAccessManager dataAccessManager = null;

    // The executor service used to create threads to record connection statistics.
    private static ExecutorService executor = null;

    /**
     * ARepository instance. This is used to handle basic resource storage operations of current
     * versions.
     */
    private Repository repository;
    /**
     * AVersionRepository instance. This is used to handle resource storage operations of archived
     * (old) versions.
     */
...
//    private JdbcDirectory jdbcDir;
    ...
    /**************************************/
    private static Map<String, TempResource> tTempResourceMap = new HashMap<String, TempResource>();

    private static Map<String, TempResource> tTempResourcePathMap = new HashMap<String, TempResource>();

    //private static Map<String,TempResource> ttTempResourceMap=new HashMap<String, TempResource>();
   ...
   // private static Map<String, TempResource> tTempResourcePathMap=new HashMap<String, TempResource>();
    private static class TempResource  {
        ...
        public TempResource(String suggestedPath, Resource resource,String transactionId){
    ...
    public String put(String suggestedPath1, Resource resource1, String transactionID) {

        TempResource tempResource=new TempResource(suggestedPath1,resource1,transactionID);
        tTempResourceMap.put(transactionID,tempResource);
        tTempResourcePathMap.put(suggestedPath1,tempResource);

        if(tTempResourceMap.containsKey(transactionID)&&tTempResourcePathMap.containsKey(suggestedPath1)){
            return suggestedPath1;
        }
        else    {
            if(tTempResourcePathMap.containsKey(suggestedPath1)){
                tTempResourcePathMap.remove(suggestedPath1);
            }
            if(tTempResourceMap.containsKey(transactionID)){
                tTempResourceMap.remove(transactionID);
            }
            return "-1";
        }
    }
    ...
    public boolean prepare(String transactionID){
        ...
        TempResource tempResource=tTempResourceMap.get(transactionID);
        //TempResource tempResource=gettMappedTempResource(transactionID);
        boolean transactionSucceeded = false;


        try {
            String suggestedPath=tempResource.getSuggestedPath();
            Resource resource=tempResource.getResource();


            RequestContext context = new RequestContext(this, repository, versionRepository);
            boolean mustPutChild = false;
            // start the transaction
            /*Connection connection=beginTTransaction();
            tempResource.setConnection(connection);*/
            beginTransaction();
            ...
            //settMappedTempResource(tempResource,transactionID);
            //ttTempResourceMap.put(transactionID,tempResource);
    ...
    public boolean commit(String transactionId){
        TempResource rCurrent=tTempResourceMap.get(transactionId);
        //Connection connection=rCurrent.getConnection();
        if(rCurrent.getTransactionId().equals(transactionId)){
            ...
            try {
                ...
                tTempResourceMap.remove(transactionId);
                tTempResourcePathMap.remove(rCurrent.getSuggestedPath());
                return isSucess;
            }
        }
        else
    ...
    public EmbeddedRegistry(RegistryContext registryContext) throws RegistryException {
        ...
        this.logsDAO = dataAccessManager.getDAOManager().getLogsDAO();
        this.associationDAO = dataAccessManager.getDAOManager().getAssociationDAO();
        this.realmService = registryContext.getRealmService();
        init();
    }
...
//        registryContext.setJdbcDir(jdbcDir);
    ...
    protected void beginDBQueryLog(int level) {
        if (dbQueryLog.isDebugEnabled()) {
    ...
    protected void endDBQueryLog(int level) {
        if (dbQueryLog.isDebugEnabled()) {
            StackTraceElement traceElement = Thread.currentThread().getStackTrace()[level];
            String methodName = traceElement.getMethodName();
            if (!statEnabledOperations.isEmpty() && statEnabledOperations.contains(methodName)) {
    ...
    @Override
    public Resource get(String path, String transactionId) throws RegistryException {
        if(tTempResourceMap.containsKey(transactionId)&&tTempResourcePathMap.containsKey(path)){
            return tTempResourceMap.get(transactionId).getResource();
        }
        else {
