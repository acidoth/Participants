...
public class WSRegistryClientUtils {
    ...
    public static OMElement getContextToHeader(CoordinationContext coordinationContext) {

        OMFactory fac = OMAbstractFactory.getOMFactory();
        OMNamespace omNs = fac.createOMNamespace( "http://ws.apache.org/axis2/2pc", "ns");
        OMElement header = fac.createOMElement("CoordinationContext", omNs);

        OMElement coordinationtype = fac.createOMElement("headerValue", omNs);
        coordinationtype.setText(coordinationContext.getCoordinationType());
        OMElement port = fac.createOMElement("port", omNs);
        port.setText(String.valueOf(coordinationContext.getPort()));
        OMElement expires = fac.createOMElement("expires", omNs);
        expires.setText(String.valueOf(coordinationContext.getExpires()));
        OMElement endpoint = fac.createOMElement("endpoint", omNs);
        endpoint.setText(coordinationContext.getEndpointreference());
        OMElement identifier=fac.createOMElement("identifier",omNs);
        identifier.setText(coordinationContext.getIdentifier());


        header.addChild(identifier);
        header.addChild(endpoint);
        header.addChild(coordinationtype);
        header.addChild(port);
        header.addChild(expires);
