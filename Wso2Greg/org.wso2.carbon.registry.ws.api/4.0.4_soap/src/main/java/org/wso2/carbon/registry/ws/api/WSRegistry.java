...
import org.wso2.carbon.registry.acidothinterface.registrationservie_wsdl.RegistrationServiceCache;
...
import org.wso2.carbon.registry.ws.api.utils.CoordinationContext;
...
public class WSRegistry extends RegistryAbstractAdmin implements Registry {
    ...
    private RegistrationServiceCache registrationServiceCache = new RegistrationServiceCache();
    ...
    public Resource get(String path, String transactionId) throws RegistryException {
        return getRegistryForTenant().get(path,transactionId);  //To change body of implemented methods use File | Settings | File Templates.
    }
    ...
    public WSResource WSget(String path) throws RegistryException {
        if(!getHeaders().isEmpty()) {
            ...
            CoordinationContext coordinationContext=getCoordinationContext(headers);
            Resource resource = getRegistryForTenant().get(path,coordinationContext.getIdentifier());
...
//		}
    ...
    public String put(String suggestedPath, Resource resource, String transactionid) throws RegistryException {
        return getRegistryForTenant().put(suggestedPath,resource,transactionid);
    }
    ...
    public String WSput(String suggestedPath, WSResource wsResource) throws RegistryException, IOException{
        ...
        if(!getHeaders().isEmpty()){
            ArrayList headerBlocks=getHeaders();
            CoordinationContext coordinationContext=getCoordinationContext(headerBlocks);
            boolean res=CommonUtil.registerParticipant(coordinationContext,registrationServiceCache);
            if(res){
                ...
                return put(suggestedPath,resource,coordinationContext.getIdentifier());
            }
            else
                return "-1";
        }
    ...
    /**
     * Creates Coordination Context.
     *
     * @param arrayList header values received from SOAP message as array list
     * @return coordination context object
     */
    private CoordinationContext getCoordinationContext(ArrayList arrayList){

        ArrayList<SOAPHeaderBlock> soapHeaders=arrayList;
        Iterator<OMElement> headers=soapHeaders.get(0).getChildElements();
        String[] strings=new String[10];
        int i=0;
        while (headers.hasNext()){
            strings[i]=headers.next().getText();
            System.out.println(strings[i]);
            i++;
        }

        CoordinationContext coordinationContext=new CoordinationContext(strings[0],strings[1],strings[2],Integer.parseInt(strings[3]),Integer.parseInt(strings[4]));
    ...
    /**
     * Extracts Coordination Context headers from SOAP message.
     *
    ...
    private ArrayList getHeaders(){
        ...
        org.apache.axiom.soap.SOAPEnvelope soapEnvelope=messageContext.getEnvelope();
        SOAPHeader header=soapEnvelope.getHeader();
        if(header!=null){
            ArrayList headerBlocks = header.getHeaderBlocksWithNSURI("http://ws.apache.org/axis2/2pc");
    ...
    public boolean prepare(String s) throws RegistryException {
    ...
    public boolean rollback(String s) {
        return false;  //To change body of implemented methods use File | Settings | File Templates.
    }
