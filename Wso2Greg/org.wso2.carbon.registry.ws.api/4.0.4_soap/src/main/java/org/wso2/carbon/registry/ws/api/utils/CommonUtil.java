...
import org.wso2.carbon.registry.acidothinterface.soap2pcserver.main.RegistrationAsyncClient;
...
//                    os = new FileOutputStream(tempFile);
    ...
    public static boolean registerParticipant(CoordinationContext cContext, RegistrationServiceCache registrationServiceCache){

        RegistrationAsyncClient registrationAsyncClient = null;
        boolean result = false;
        try {
            RegistrationService registrationService = registrationServiceCache.getRegistrationServicePort(cContext.getEndpointreference());
            registrationAsyncClient = new RegistrationAsyncClient(registrationService);
            result = registrationAsyncClient.register(cContext.getIdentifier(), Constants.coordinationServiceAddress,cContext.getPort(),cContext.getCoordinationType());
