...
import org.wso2.carbon.registry.acidothinterface.registrationclient.RegistrationAsyncClient;
...
//                    os = new FileOutputStream(tempFile);
    ...
    public static boolean registerParticipant(CoordinationContext cContext){
        org.wso2.carbon.registry.acidothinterface.thriftgen.registration.CoordinationContext coordinationContext = new org.wso2.carbon.registry.acidothinterface.thriftgen.registration.CoordinationContext();
        EndPointReference registrationEPR = new EndPointReference();
        EndPointReference participantEPR = new EndPointReference();

        registrationEPR.setAddress(cContext.getEndpointreference());
        registrationEPR.setPrivateInstance(cContext.getPort());
        coordinationContext.setIdentifier(cContext.getIdentifier());
        coordinationContext.setCoordination_type(cContext.getCoordinationType());
        coordinationContext.setRegistrationEPR(registrationEPR);
        coordinationContext.setExpires(cContext.getExpires());

        participantEPR.setAddress(Constants.coordinationAddress);
        participantEPR.setPrivateInstance(Constants.coordinationPort);

        RegistrationAsyncClient coordinationServiceClient = new RegistrationAsyncClient(cContext.getEndpointreference(), cContext.getPort());
        System.out.println("Put Done "+cContext.getIdentifier());
        return coordinationServiceClient.Register(coordinationContext, "2PC", participantEPR);
    }
